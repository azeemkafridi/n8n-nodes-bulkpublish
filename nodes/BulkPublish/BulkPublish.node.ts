  ...
  description: INodeTypeDescription = {
    ...
    properties: [
      ...
      },
      {
        ...
        default: '',
        displayOptions: { show: { resource: ['rssFeed'], operation: ['create'] } },
        description: 'How each feed item becomes a post, as a JSON object — e.g. {"template": "{title}\\n\\n{link}", "mediaField": "auto", "hashtags": "#blog", "channelOverrides": {"2": {"template": "{title} {link}"}}}. Fields: template (tokens {title} {link} {description} {content} {author} {categories} {feedName}, plus any extra field on the feed item as {fieldName}), mediaField (none/image/video/auto), articleImage (when_needed default/always/never — when the item has no picture of its own, take one from the article it links to; when_needed does so only if a channel on the feed is on a platform that cannot publish without an image, and the picture attaches to the post so every channel gets it), stripHtml (default true), truncate (smart/hard/skip), hashtags, channelOverrides (per-channel text overrides keyed by channel ID). Leave empty for the server default ({title} then the link, no enclosure attached, and an article image taken when a channel needs one).',
      },
      {
      ...
      },
      {
        ...
        default: '',
        displayOptions: { show: { resource: ['rssFeed'], operation: ['update'] } },
        description: 'How each feed item becomes a post, as a JSON object — e.g. {"template": "{title}\\n\\n{link}", "mediaField": "auto", "hashtags": "#blog", "channelOverrides": {"2": {"template": "{title} {link}"}}}. Fields: template (tokens {title} {link} {description} {content} {author} {categories} {feedName}, plus any extra field on the feed item as {fieldName}), mediaField (none/image/video/auto), articleImage (when_needed default/always/never — when the item has no picture of its own, take one from the article it links to; when_needed does so only if a channel on the feed is on a platform that cannot publish without an image, and the picture attaches to the post so every channel gets it), stripHtml (default true), truncate (smart/hard/skip), hashtags, channelOverrides (per-channel text overrides keyed by channel ID). Leave empty for the server default ({title} then the link, no enclosure attached, and an article image taken when a channel needs one). Pass the literal value null to clear the mapping back to the server default. Leave empty to keep current.',
      },
      {
        displayName: 'Enabled',
        name: 'rssFeedEnabled',
